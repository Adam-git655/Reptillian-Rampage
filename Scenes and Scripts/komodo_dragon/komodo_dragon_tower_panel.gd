...
@onready var KomodoDragonTower = preload("res://Scenes and Scripts/komodo_dragon/komodo_dragon_tower.tscn")
@onready var reptiles_path = get_tree().get_current_scene().get_node("Reptiles")
var tower_price = 180

func _ready() -> void:
	$"Komodo-dragon/TowerPrice".text = str(tower_price)
...
func _on_gui_input(event: InputEvent) -> void:
	if Main.scales >= tower_price:
		...
		if event is InputEventMouseButton and event.button_mask == 1:
			...
			TempKomodoDragonTower.global_position = get_global_mouse_position()
			...
			if get_global_mouse_position().y >= 555 and get_global_mouse_position().x >= 955 and get_global_mouse_position().x <= 1050:
				...
				if is_point_in_collision_polygon(get_global_mouse_position(), get_tree().get_root().get_node("@Node2D@2/PlaceableArea/LandArea1")) or is_point_in_collision_polygon(get_global_mouse_position(), get_tree().get_root().get_node("@Node2D@2/PlaceableArea/LandArea2")):
					...
					if overlapping_towers.size() < 1:		
						...
						$"Komodo-dragon/PlaceSoundEffect".play()
		else:
			if get_child_count() > 1:
				get_child(1).queue_free()
